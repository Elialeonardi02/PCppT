...
int MAIN_WITH_ARGV()
{
    ...
    if (in(u"-h"_S, ::argv) || in(u"--help"_S, ::argv)) {
        print(uR"(A Python implementation of pqmarkup to HTML converter.
  ...
  -f OUTPUT_FILE, --file OUTPUT_FILE
    ...
    while (i < ::argv.len()) {
        ...
        if (!::argv[i].starts_with(u"-"_S)) {
            try
            {
                args_infile = File(::argv[i], u"utf-8-sig"_S);
            }
            catch (...)
            {
    ...
    try
    {
        if (in(u"-f"_S, ::argv)) {
            ...
            args_outfile = FileWr(outfile_name, u"utf-8"_S);
        }
