...
public:
    ...
    template <typename T3 = decltype(0)> String to_html(const String &instr, FileWr* const outfilef = nullptr, const T3 &outer_pos = 0)
    {
            ...
            if (ch == u'‘') {
                ...
                if (in(prevc, u"PР"_S)) {
                    ...
                    public:
                        ...
                        TableRow(const String &style) :
                            style(style)
                        {
                        }
                    };
                    Array<TableRow> table;
                    auto j = startqpos + 1;

                    auto parse_color = [&exit_with_error, &instr, &j, &translate_color]()
                    {
                        j++;
                        if (instr[j] != u'(')
                            exit_with_error(u"Expected `(` after `C`"_S, j);
                        j++;
                        if (instr[j] != u'-')
                            exit_with_error(u"Expected `-` after `C(`"_S, j);
                        auto s = j + 1;
                        j = instr.findi(u")"_S, s);
                        return u"background-color: "_S & translate_color(instr[range_el(s, j)]);
                    };
                        ...
                        if (ch == u'‘') {
                            ...
                            while (j < endrow) {
                                ...
                                if (ch == u'‘') {
                                    ...
                                    if (cell_style != u"")
                                        style &= (u"; "_S * (style != u"")) & cell_style;
                                    hor_col_align = u""_S;
                                    ver_col_align = u""_S;
                                    cell_style = u""_S;
                                    table.last().cells.append(TableCell(to_html(instr[range_el(j + 1, end_of_column)], nullptr, j + 1), (header_row ? u"th"_S : u"td"_S) & (style != u"" ? u" style=\""_S & style & u"\""_S : u""_S)));
                                    j = end_of_column;
                                }
                                else if (in(ch, u"CС"_S))
                                    ...
                                    if (table.last().cells.empty())
                                        exit_with_error(u"Wrong table column span marker \"-\""_S, j);
                                    table.last().cells.append(TableCell(u""_S, u"-"_S));
                                }
                                else if (ch == u'|') {
                                    if (table.len() == 1)
                    ...
                    for (auto y : range_el(table.len() - 1, -1).step(-1))
                        for (auto x : range_el(table[y].cells.len() - 1, -1).step(-1))
                            if (in(table[y].cells[x].attrs, make_tuple(u"-"_S, u"|"_S))) {
                                auto xx = x;
                                auto yy = y;
                                while (true)
                                    if (table[yy].cells[xx].attrs == u'-')
                                        xx--;
                                    else if (table[yy].cells[xx].attrs == u'|')
                                        yy--;
                                    else
                                        break;
                                if (xx < x)
                                    table[yy].cells[xx].attrs &= u" colspan=\""_S & String(x - xx + 1) & u"\""_S;
                                if (yy < y)
                                    table[yy].cells[xx].attrs &= u" rowspan=\""_S & String(y - yy + 1) & u"\""_S;
                                for (auto xxx : range_ee(xx, x))
                                    for (auto yyy : range_ee(yy, y))
                                        if (make_tuple(xxx, yyy) != make_tuple(xx, yy))
                                            table[yyy].cells[xxx].attrs = u""_S;
                            }
                    ...
                    for (auto &&row : table) {
                        outfile.write(u"<tr"_S & (row.style != u"" ? u" style=\""_S & row.style & u"\""_S : u""_S) & u">"_S);
...
template <typename T1, typename T3 = decltype(false), typename T4 = decltype(false)> auto to_html(const T1 &instr, FileWr* const outfilef = nullptr, const T3 &ohd = false, const T4 &habr_html = false)
{
